...
def getPossiblePositions(n: int,queen: int,currentPositions: list[list[str]],allPossiblePositions: list[list[str]]) :
    # Base Case or all queens have been safely placed
    if(n == queen-1) :
        positions = []
        for i in range (0, len(currentPositions)):
            positions.append("".join(currentPositions[i]))
        allPossiblePositions.append(positions)
        return
