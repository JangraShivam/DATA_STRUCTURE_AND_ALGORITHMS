...
def isPossible(days :int, bloomDay : List[int], m : int, k : int) -> bool:
    counter = 0 # counter for flower in current bouqets

    # iterate over bloomday array or make bouqets
    for i in range(0,len(bloomDay)) :
        # if flower is bloomed then add in current bouqet
        if(bloomDay[i] <= days) : 
            counter+=1
        # if not then we can not make current bouqet because adjacent flowers are needed.
        else :
            counter = 0
        ...
        if(counter == k) :
            ...
            m-=1 # decrease required bouqet count
            # check if we made all the bouqet
            if(m == 0):
                return True
...
def minDays(bloomDay: List[int], m: int, k: int) -> int:
    ...
    print(requiredFlowers,numberOfFLowers)
    # Check if it is possible to make bouqets and if not return -1
    if(numberOfFLowers < requiredFlowers):
        return -1
    ...
    while (minDays <= maxDays) :
        ...
        mid = (minDays + maxDays)//2 # possible answer
...
# main program
size = int(input("Enter size of bloomDay array : "))
bloomDay = list(map(int, input("Enter bloomDay array elements : ").strip().split()))[:size]
m = int(input("Enter m : "))
k = int(input("Enter k : "))

answer = minDays(bloomDay,m,k)
print(answer)