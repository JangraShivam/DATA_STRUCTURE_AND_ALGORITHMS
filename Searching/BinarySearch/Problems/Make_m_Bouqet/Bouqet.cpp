...
int minDays(vector<int>& bloomDay, int m, int k) {
    ...
    int numberOfFLowers = bloomDay.size();

    // Required flowers for making bouqets
    long int requiredFlowers = (long int)m*(long int)k;

    // Check if it is possible to make bouqets and if not return -1
    if((long int)numberOfFLowers < requiredFlowers) return -1;
...
int main(){
    ...
    cout << "Enter size of bloomDay array : " << endl;
    int size;
    cin >> size;

    vector<int> bloomDay(size);
    cout << "Enter bloomDay array elements : " << endl;
