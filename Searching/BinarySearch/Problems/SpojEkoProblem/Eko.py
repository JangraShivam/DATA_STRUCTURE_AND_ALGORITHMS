...
def getWood(Trees, height):
    ...
    for h in Trees :
        woodAmount += max(0,(h - height))
    
    return woodAmount


def getMaxHeightOfBlade(Trees, requiredWoodAmount) :
    start = 0
    end = max(Trees)
...
numberOfTress = int(input())
requiredWoodAmount = int(input())
Trees = list(map(int, input().strip().split()))
print(getMaxHeightOfBlade(Trees,requiredWoodAmount))